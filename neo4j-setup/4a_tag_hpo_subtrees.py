#!/usr/bin/env python3
"""
exclusively-neo4j/4a_tag_hpo_subtrees.py
────────────────────────────────────────────────────────────────────────────────
Script 4a — Tag every HPO term with a category (symptom / lab_finding / sign)
by walking subtrees of the Human Phenotype Ontology.

Inputs  (DATA_DIR/processed/ and ONTOLOGIES_DIR/)
──────────────────────────────────────────────────
    symptom_index.json      written by 2_parse_ontologies.py
    hp.obo                  raw HPO ontology file

Output  (DATA_DIR/processed/)
──────────────────────────────
    symptom_categories.json   { hp_id → "symptom" | "lab_finding" | "sign" }

Usage
─────
    # from exclusively-neo4j/
    uv run 4a_tag_hpo_subtrees.py
"""
...
SYMPTOM_ROOTS = {
    ...
    "HP:0002086",  # Respiratory system abnormality (dyspnea, cough — patient-reported)
}
...
def main():
    ...
    # Validate subtree roots
    for root_id in sorted(ALL_ROOTS):
        term = ont.get(root_id)
        if term is None or term.obsolete:
            print(
                f"WARNING: subtree root {root_id} not found or obsolete in this "
                f"version of hp.obo — its descendants will be tagged 'sign'"
            )

    # Build ancestor sets for each root (descendants of root)
    ...
    for root_id in ALL_ROOTS:
        ...
        if term is None or term.obsolete:
            ...
            continue
        # subclasses() returns all descendants
    ...
    def tag_term(hp_id: str) -> str:
        ...
        # Collect ancestor IDs including self
        ancestor_ids = {t.id for t in term.superclasses(distance=None, with_self=True)}

        # Walk: first matching root wins
        # Check symptom roots first, then lab roots, default to sign
    ...
    # Check 2: all three categories present
    ...
    # Check 4: HP:0001939 descendants include at least one lab_finding
    ...
    # Check 5: no null or empty string values
